...
bool ClientSession::SendQueuePush(std::shared_ptr<Packet> packet) {
    ...
    if (closing.load(std::memory_order_acquire)) { // 다시 한번 확인
        return false;
    }
...
std::shared_ptr<Packet> ClientSession::SendQueuePop(){
    ...
    std::shared_ptr<Packet> packet = send_queue_.pop();
...
void recv_run(){
    ...
    if (대충 종료해야하는 조건) {
        HandleTransportException();
        SendQueueCV_NotifyOne(); // 여기서 cv.notify_one()
    }
}
...
void send_run() {
    ...
    if (closing == true) {
        // 대충 종료하는 코드
        // 여기다가 ClientSockShutdown() 호출하는 코드 넣어도 되나?
    }
}
```
...
물론 실제로는 가짜 기상(Spurious Wakeup) 문제도 해결해야하지만..
이 정도면 대충 감 오는듯.
...
void send_run(){
    ...
    if (대충 종료해야하는 조건) {
        HandleTransportException();
        ClientSockShutdown(); // 추후에 신설할 RAII 객체의 메서드. shutdown() 함수를 호출해줌.
    }
}
...
void recv_run() {
    ...
    if (대충 종료해야하는 조건) {
        HandleTransportException();
        // 여기다 SendQueueCV_NotifyOne() 넣어버려도 되나?
    }
}
```

## 에코 서버 / 채팅 서버의 전체적인 실행 흐름 정리(이거 보면서 해라)

(에코 서버)

```text
(위에서 아래로, 그냥 Run() 함수의 흐름이라고 생각하면 됨.)
    ├── RecvPacket()
    ├── NetState를 보고 오류 있으면 HandleTransportException()
    |     ├── MarkClosing()
    |     ├── 경우에 따라 예외 처리
    |     |      ├── transport error
    |     |      ├── peer exit
    |     |      └── protocol error
    |     |            ├── 얘만 특수한 예외처리를 함.
    |     |            └── HEADER_ERROR 패킷을 SendPacket() 
    |     └── RemoveClient() - MarkClosing() 성공했을 때만
    └── 오류 없는 경우에는 HandleRecvPacket()
          ├── type == NICKNAME_CHANGE
          |      ├── 닉네임 유효성 검사
          |      |     ├── 유효한 경우 닉네임 변경 후 NICKNAME_CHANGE_SUCCESS SendPacket()
          |      |     └── 유효하지 않은 경우 실패 원인을 담아서 NICKNAME_CHANGE_FAILED SendPacket()
          |      └── 함수 종료
          ├── type == CHAT_MESSAGE
          |      ├── 본인 클라이언트에 에코 메시지 SendPacket()
          |      └── 오류 있으면 HandleTransportException()
          |            ├── MarkClosing()
          |            ├── 경우에 따라 예외 처리
          |            |      ├── transport error
          |            |      ├── peer exit - 여기서는 필요 없음(recv 전용)
          |            |      └── protocol error - 혹시 몰라서 함
          |            |            ├── 얘만 특수한 예외처리를 함.
          |            |            └── HEADER_ERROR 패킷을 SendPacket() 
          |            └── RemoveClient() - MarkClosing() 성공했을 때만
          └── type == HEADER_ERROR
                ├── 로그 출력
                ├── MarkClosing()
                └── MarkClosing 성공했을 때만 RemoveClient()
```

(브로드캐스트 채팅 서버)

```text
(위에서 아래로, 그냥 RecvRun() 함수의 흐름이라고 생각하면 됨.)
    ├── RecvPacket()
    ├── NetState를 보고 오류 있으면 HandleTransportException()
    │    └── (에코 서버의 로직과 동일)
    └── 오류 없는 경우에는 HandleRecvPacket()
          ├── type == NICKNAME_CHANGE
          |      ├── 닉네임 유효성 검사
          |      |     ├── 유효한 경우 닉네임 변경 후 NICKNAME_CHANGE_SUCCESS 패킷을 해당 ClientSession의 Send Queue에 Push()
          |      |     └── 유효하지 않은 경우 실패 원인을 담아서 NICKNAME_CHANGE_FAILED 패킷을 해당 ClientSession의 Send Queue에 Push()
          |      └── 함수 종료
          ├── type == HEADER_ERROR
          |     ├── 로그 출력
          |     ├── MarkClosing()
          ...
          └── type == CHAT_MESSAGE
                └── ClientManager::Broadcast()
                      ├── snapshot 복사
                      ├── 송신자와 같은 세션이면 건너뜀
                      └── 각각의 Send Queue에 Push()
(아직 closing == true라면 종료하는 로직은 없음)
```

```text
(위에서 아래로, 그냥 SendRun() 함수의 흐름이라고 생각하면 됨.)
    ├── 만약 Send Queue에 패킷이 하나라도 들어있다면
    |     └── 해당 패킷을 Pop() 후 SendPacket()
    |           └── 오류가 있으면 HandleTransportException()
    └── 만약 Send Queue에 패킷이 하나도 없다면
          └── blocing 상태로 대기
(아직 closing == true라면 종료하는 로직은 없음)
```
...
3. send / recv thread 도입 관련 의존성
4. `Broadcast` 도입 관련 의존성
...
### 7-3. send / recv thread 도입 관련 의존성

### 7-4. `Broadcast` 도입 관련 의존성
...
#### 입원 전 기록용, 설계 문서에 뭐 고려 안되어있나

이게 내가 양극성 장애 / 불안 장애 / 망상 장애 / 해리 장애 관련으로 입원할 가능성이 커서
일단 지금 당장은 공부 중지하고 좀 쉬어야 할 것 같음. 진짜 입원해야 할 각이 매우 날카로움..
더 하고싶긴 한데 솔직히 이건 내가 생각해도 입원하는게 맞음.

그래서 일단 지금 생각나는거 싹 다 대충 적고 감.

1. 일단 헤더의 닉네임을 파싱하는 그런게 안되어있음. 하지만 이건 헤더의 닉네임 필드를 memcpy로 통째로 복사해서 Send Queue로 넘기면 해결됨.
    - 결국 패킷 타입 별 처리를 하기 때문에 추후 다른 범위의 채팅 기능이 들어와도 해당 패킷 타입의 처리를 별도로 만들어주면 되기 때문에 현재의 브로드캐스트로써는 해당 패킷을 그대로 Send Queue로 넘기면 됨.
    - 추후에 다른 범위의 채팅 기능은 패킷 헤더를 수정한 후 broadcast() 함수에 넘길 생각. 
    - 이거 정책(불변식) 위반 아니냐고 할 수 있는데 해당 패킷이 불변인건 broadcast() 호출 이후, 즉 해당 시점에 어떤 Send Queue에라도 전달될 가능성이 존재하지 않은 경우임.
      - 즉, 브로드캐스트 기능에서는 broadcast() 호출 전에는 해당 패킷의 내용을 수정할 수 있다는 것.
2. 모르겠다.. 이거 말고는 기억 안남.. 그냥 지피티나 클로드가 한거 보고 하자..