...
그리고 왜 굳이 Recv 스레드를 main 스레드가 담당하냐면
Recv 스레드는 blocking 해제 조건이 매우 명확하기 때문임.

결국 `TryMarkClosing()` 함수의 `shutdown()` 호출로 명확히 blocking 해제가 가능하고,
서버에서 `SOCKET_ERROR`나 `0`이 `Recv`되면 다른 blocking되는 수단 없이 그대로 blocking이 해제가 되기 때문임.

그래서 Recv 스레드만 종료되어도 프로그램이 종료될 수 있게 함.
