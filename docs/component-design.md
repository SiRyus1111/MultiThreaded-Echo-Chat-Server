...
4. 패킷 송수신 절차는 RecvPacket() / SendPacket()으로 분리한다.
5. Run()은 Echo Server의 고수준 흐름만 제어한다.
6. 송수신 결과는 bool 하나가 아니라 NetState로 표현한다.
7. ClientSession의 물리적 생존, ClientSession의 논리적 종료, ClientSession에 대한 ClientManager의 관리 목록 제거를 분리한다.
```
...
추후 `SessionID`가 도입되면 다음처럼 확장할 수 있습니다.
...
생성자는 `ClientSocket`의 소유권과 클라이언트 주소 정보를 받아
하나의 `ClientSession`으로 묶습니다.

핵심은 `ClientSocket`의 소유권 이동입니다.
...
accept()로 얻은 client socket
  ...
  → 이후 해당 socket은 ClientSession이 소유한다
```
...
ClientSession
  → Manager_wp.lock()
  → ClientManager가 살아있으면 RemoveClient(shared_from_this()) 호출
...
자신을 관리하는 ClientManager에게 관리 목록 제거를 요청한다.
```
...
```text
RemoveThisClient()
  → ClientManager에 제거 요청, 즉 ClientManager::RemoveClient() 호출 요청
...
`client_mutex`는 `clients` 컨테이너를 보호하기 위한 mutex입니다.
...
중요한 점은 `client_mutex`가 모든 송신 작업을 보호하는 mutex가 아니라는 것입니다.

실제 `send()`는 block될 수 있으므로,
추후 Broadcast 단계에서는 `client_mutex`를 잡은 상태에서 `send()`를 오래 수행하지 않는 구조를 목표로 합니다.
...
2. client_mutex를 잡는다.
3. clients 컨테이너에 shared_ptr<ClientSession>을 추가한다.
```
...
`AddClient()`에서 중요한 점은
단순히 vector에 push하는 것만이 아니라는 점입니다.

이 함수는 다음 관계를 완성합니다.

```text
ClientManager
  → ClientSession을 shared_ptr로 관리
...
`RemoveClient()`는 특정 `ClientSession`을 `clients` 컨테이너에서 제거하는 함수입니다.
...
이 구조를 사용하는 이유는,
`client_mutex`를 잡은 상태에서 실제 `send()`를 오래 수행하지 않기 위해서입니다.

`send()`는 block될 수 있으므로,
...
현재는 클라이언트를 `shared_ptr<ClientSession>` 또는 주소 정보로 구분할 수 있습니다.

하지만 추후 클라이언트 수가 늘고,
특정 세션을 빠르게 찾거나 로그에서 구분해야 할 필요가 생기면
`SessionID`를 도입할 예정입니다.
...
```cpp
typedef SessionID = uint64_t;
```

추후 구조는 다음처럼 확장될 수 있습니다.
...
이는 `ClientManager`의 `client_mutex`와 역할이 다릅니다.
