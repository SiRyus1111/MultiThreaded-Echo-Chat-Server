...
기존 방식은 하나의 로그를 여러 번의 << 연산으로 출력하였습니다.

따라서 서로 다른 스레드의 출력이 한 줄 내부에서 섞일 수 있습니다.

이를 줄이기 위해 프로젝트는 LineLogger를 사용합니다.
...
문자열 생성은 각 스레드가 독립적으로 수행하므로
락이 필요하지 않습니다.

반면 `std::cout`은 공유 자원이므로
출력 시점만 `std::mutex`로 보호합니다.
