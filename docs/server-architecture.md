...
이 문서는 `MultiThreaded Echo Server`의 **서버**의 전체 구조와 책임 분리를 정리합니다.
...
`ClientSession`의 멤버 변수와 핵심 함수에 대한 상세 설계는 [server-component-design.md](server-component-design.md) 참고.
...
`ClientManager`의 멤버 변수와 핵심 함수(AddClient, RemoveClient, GetClients, Broadcast)에 대한 상세 설계는 [server-component-design.md](server-component-design.md) 참고.
